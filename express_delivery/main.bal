...
service on cons {
    ...
    remote function onConsumerRecord(Shipment[] requests) returns error? {
        foreach Shipment req in requests {
            io:println("Received Express Delivery Request: ", req);

            // Add 1 day to the preferred delivery time
            string updatedDeliveryTime = check addOneDayToPreferredTime(req.preferredTime);
...
function addOneDayToPreferredTime(string preferredTime) returns string|error {
